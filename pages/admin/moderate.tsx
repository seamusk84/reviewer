...
type ReviewRow = {
  ...
  created_at: string; status: string; rating: number; estate_id: string;
};
type AreaRow = {
  id: string; county_id: string; town_id: string;
  estate_name: string; contact_email: string | null; created_at: string; status: string;
};
...
export default function ModeratePage() {
  ...
  async function load() {
    ...
    try {
      const res = await fetch("/api/admin/list-pending?token="+encodeURIComponent(token), { headers: { "x-admin-token": token } });
    ...
    <div className="container">
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <div className="grid" style={{ marginTop: 12 }}>
          <input className="select" type="password" placeholder="Admin token" value={token} onChange={e => setToken(e.target.value)} />
          <button className="btn" onClick={load} disabled={!token || loading}>{loading ? "Loading…" : "Load pending"}</button>
        </div>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        {reviews.map(r => (
          <div key={r.id} className="review">
            <div className="small muted">{new Date(r.created_at).toLocaleString()} • Estate ID: {r.estate_id} • Rating: {r.rating}</div>
