...
type Review = {
  id: string; rating: number; title: string | null; body: string | null; created_at: string; estate_id: string;
};
...
function normalizeCounty(raw: string): string {
  ...
  const map: Record<string,string> = {
    "londonderry":"derry","derry/londonderry":"derry","co derry":"derry","queen's county":"laois","tipp":"tipperary"
  };
...
async function fetchCSV(path: string): Promise<string[][]> {
  try {
    ...
    return text.replace(/^\uFEFF/,"").split(/\r?\n/).map(r=>r.trim()).filter(r=>r.length>0).map(r=>r.split(",").map(c=>c.trim()));
  } catch { return []; }
}
...
class ErrorBoundary extends React.Component<{children:React.ReactNode},{hasError:boolean}> {
  ...
  render(){ return this.state.hasError ? <div className="card"><div className="small">Section failed to render.</div></div> : this.props.children; }
}

/** ---------- Hooks (DB first, CSV fallback) ---------- */
function useCounties(){
  const [counties, set] = React.useState<County[]>(COUNTY_LIST);
  const [loading, setLoading] = React.useState(true);
  React.useEffect(()=>{ let cancel=false; (async()=>{
    try{
...
function useTowns(countyId: string | null){
  ...
  React.useEffect(()=>{ let cancel=false; (async()=>{
    ...
    setLoading(true);
    try{
      const {data,error}=await supabase.from("towns").select("id,name,county_id").eq("county_id",countyId).order("name");
      if(!cancel && !error && Array.isArray(data) && data.length){ set(data as Town[]); return; }
      const rows=await fetchCSV("/data/places.csv");
      const body = rows[0]?.[0]?.toLowerCase().includes("town") ? rows.slice(1) : rows;
      const fromCsv: Town[] = body.filter(r=>r.length>=2).map(([townName,countyName])=>({
        id: slug(`${townName}-${normalizeCounty(countyName)}`),
        name: townName,
        county_id: normalizeCounty(countyName),
      })).filter(t=>t.county_id===countyId).sort((a,b)=>a.name.localeCompare(b.name));
      if(!cancel) set(fromCsv);
    } finally { if(!cancel) setLoading(false); }
  })(); return ()=>{cancel=true}; },[countyId]);
...
function useEstates(town: Town | null){
  ...
  React.useEffect(()=>{ let cancel=false; (async()=>{
    ...
    setLoading(true);
    try{
      const {data,error}=await supabase.from("estates").select("id,name,town_id").eq("town_id",town.id).order("name");
      if(!cancel && !error && Array.isArray(data) && data.length){ set(data as Estate[]); return; }
      const rows=await fetchCSV("/data/estates.csv");
      const body = rows[0]?.[0]?.toLowerCase().includes("estate") ? rows.slice(1) : rows;
      const fromCsv: Estate[] = body.filter(r=>r.length>=2).map(([estateName,townName])=>({
        id: slug(`${estateName}-${townName}`),
        name: estateName,
        town_id: slug(`${townName}-${town.county_id}`),
      })).filter(e=>e.town_id===town.id).sort((a,b)=>a.name.localeCompare(b.name));
      const withWhole = fromCsv.length ? fromCsv : [{ id: `${town.id}__whole`, name: WHOLE_AREA, town_id: town.id }];
      if(!cancel) set(withWhole);
    } finally { if(!cancel) setLoading(false); }
  })(); return ()=>{cancel=true}; },[town]);
...
function useReviews(estateId: string | null){
  ...
  React.useEffect(()=>{ let cancel=false;
    if(!estateId || !isUUID(estateId)){ setItems([]); setError(null); setLoading(false); return; }
    (async()=>{ setLoading(true); setError(null);
      try{
        const {data,error}=await supabase.from("reviews")
          .select("id,rating,title,body,created_at,estate_id")
          .eq("estate_id",estateId).order("created_at",{ascending:false}).limit(25);
        if(!cancel){
...
/** ---------- Suggest Area form ---------- */
function SuggestAreaForm({ countyId, townId, counties, towns }:{
  countyId: string | null; townId: string | null; counties: County[]; towns: Town[];
}){
  ...
  async function submit(){
    try{
      ...
      const { error } = await supabase.from("area_suggestions").insert({
        county_id: county!.id, town_id: town!.id, estate_name: estateName, contact_email: email || null, status: "pending"
      });
      if (error) throw error;
      setStatus("ok"); setEstateName(""); setEmail("");
...
function NewsTicker(){
  ...
  React.useEffect(()=>{ let cancel=false; (async()=>{
    try{ const res=await fetch("/api/news"); const j=await res.json(); if(!cancel) setItems(Array.isArray(j?.items)?j.items:[]); }
    catch{ if(!cancel) setItems([]); }
  })(); return ()=>{cancel=true}; },[]);
  if(!items.length) return null;
  return (
    <div style={{overflow:"hidden",whiteSpace:"nowrap",border:"1px solid rgba(2,6,23,.08)",borderRadius:12,padding:"8px 12px",marginBottom:16,background:"#fff"}}>
      ...
      <div className="container">
        ...
        <ErrorBoundary>
          <main className="card">
            ...
            <div className="mt24">
              <button className="btn" disabled={!estateId} onClick={()=>window.scrollTo({top:document.body.scrollHeight,behavior:"smooth"})}>View reviews</button>
            </div>
            ...
            <section className="mt16">
              ...
              <SuggestAreaForm countyId={countyId} townId={townId} counties={countyList} towns={townList} />
            </section>
          </main>
        </ErrorBoundary>
